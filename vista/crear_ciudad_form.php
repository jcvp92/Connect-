...
if (isset($_GET["Codigo_ciudad"])) {
    $Cliente = ($_GET["Codigo_ciudad"]);
    $cliente = CiudadesControlador::editarCiudad($Cliente);
}
?>
        ...
        <div class="col-md-3" style="height: 900px; background-color: #f8fafb">
            <div class="container">
                ...
                <div id="Navbar2" class="d-grid gap-2 col-12 mx-auto mt-5 float-none">
                    ...
                    <a href="cerrar_sesion.php" class="btn  btn-lg" style="font-size: 18px; font-family: Bahnschrift"><i class="fa fa-power-off  fa-md mr-2"></i>Cerrar sesión</a>
                </div>
            </div>
            ...
            <div class="encabezado container-fluid mt-5 ml-4 col-md-9 col-lg-9">
                ...
                <div class="container-fluid mt-1">
                    ...
                    <div class=" float-left col-md-12 col-lg-12 col-sm-12">
                        ...
                        <form class="row g-3" id="editUser" action="crear_ciudad_logic.php" method="POST" enctype="multipart/form-data">
                            <?php
                            if (is_null($cliente)) { ?>
                                <h1 id="sesion" class="float-center  pl-5 mb-2" style="font-family: Champagne; margin-top : -80px;">Crear nuevo</h1>
                            <?php } else { ?>
                                <div class="ml-5">
                                    <h1 style="font-family: Champagne; margin-top: -80px; font-size:45px; " id="sesion" class="d-flex justify-content-lg-center ml-5  text-dark">editar <?=$cliente->getCodigo_ciudad(); ?>
                                    </h1>
                                    <input type="hidden" name="Codigo_ciudad" value="<?=$cliente->getCodigo_ciudad(); ?>">
                                </div>
                            <?php } ?>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-id-card form-control-icon"></span>
                                    <input type="number" name="txtCodigo_ciudad" class="form-control" readonly value="<?=$cliente->getCodigo_ciudad() ?>" placeholder="Identificación" id="inputEmail4">
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-map-marker form-control-icon"></span>
                                    <input type="text" name="txtUbicacion" class="form-control" required value="<?=$cliente->getUbicacion() ?>" placeholder="Ubicacion" id="Ubicacion">
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-user-secret form-control-icon"></span>
                                    <select class="form-select form-control" name="txtdepartamentos" required aria-label="Default select example" id="inputEmail4">
                                        ...
                                        <?php foreach ($filassedes as $valor) {  ?>
                                            <option value=<?php echo $valor["departamentos"]; ?> <?=$cliente->getdepartamentos() == $valor["departamentos"] ? "selected" : ""?>><?php echo $valor['departamentos']; ?></option>
                                        <?php  } ?> -->
                                    </select>
                                </div>
                            </div>

...
$filassedes = EquiposControlador::getTipo();

$equipo = null;

if (isset($_GET["id_equipo"])) {
    $Equipo = ($_GET["id_equipo"]);
    $equipo = EquiposControlador::editarEquipo($Equipo);
}
?>
            ...
            <div class="encabezado container-fluid mt-5 ml-4 col-md-9 col-lg-9">
                ...
                <div class="container-fluid mt-1">
                    ...
                    <div class=" float-left col-md-12 col-lg-12 col-sm-12">
                        ...
                        <form class="row g-3" id="editUser" action="crear_equipo_logic.php" method="POST" enctype="multipart/form-data">

                            <?php

                            if (is_null($equipo)) { ?>
                                <h1 id="sesion" class="float-center  pl-5 mb-2" style="font-family: Bahnschrift; margin-top : -80px;">Crear nuevo equipo</h1>
                            <?php } else { ?>
                                <div class="ml-5">
                                    <h1 style="font-family: Bahnschrift; margin-top: -80px; font-size:45px; " id="sesion" class="d-flex justify-content-lg-center ml-5  text-dark">editar <?=$equipo->getModelo(); ?> <?=$equipo->getCodigo_interno(); ?>
                                    </h1>
                                    <input type="hidden" name="id_equipo" value="<?=$equipo->getid_equipo(); ?>">
                                </div>
                            <?php } ?>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-id-badge form-control-icon"></span>
                                    <input type="number" name="txtid_equipo" class="form-control" readonly value="<?=$equipo->getid_equipo() ?>" placeholder="Identificación" id="inputEmail4">
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-wpexplorer form-control-icon"></span>
                                    <input type="text" name="txtModelo" class="form-control" required value="<?=$equipo->getModelo() ?>" placeholder="Modelo" id="Modelo">
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-wrench form-control-icon"></span>
                                    <input type="text" name="txtFabricante" class="form-control" required value="<?=$equipo->getFabricante() ?>" placeholder="Fabricante" id="inputEmail4">
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-code form-control-icon"></span>
                                    <input type="text" name="txtSerial" class="form-control" required value="<?= $equipo->getSerial() ?>" placeholder="Serial" id="inputEmail4">
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-street-view form-control-icon"></span>
                                    <input type="text" name="txtUbicacion" class="form-control" required placeholder="Ubicacion" value="<?= $equipo->getUbicacion() ?>" id="inputEmail4">
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-microchip form-control-icon"></span>
                                    <input type="number" name="txtCodigo_interno" class="form-control" required placeholder="Codigo interno" value="<?= $equipo->getCodigo_interno() ?>" required>
                                </div>
                            </div>
                            <div class="col-md-6 mt-3">
                                <div class="form-group">
                                    <span class="fa fa-user-secret form-control-icon"></span>
                                    <select class="form-select form-control" name="txtid_tipo" required aria-label="Default select example" id="inputEmail4">
                                        <!-- <option selected>Sede</option> -->
                                        $filassedes = array();
                                        <?php foreach ($filassedes as $valor) {  ?>
                                            <option value=<?php echo $valor["id_tipo"]; ?> <?=$equipo->getid_tipo() == $valor["id_tipo"] ? "selected" : ""?>><?php echo $valor['id_tipo']; ?></option>
                                        <?php  } ?> -->
                                    </select>
                                </div>
                            </div>
                            <div id="edit" class="justify-content-center mt-3 mb-3 d-md-table mx-auto ">
                                <div class="form-group">
                                    ...
                                    <button type="submit" class="reg btn-lg d-flex justify-content-center d-md-table mx-auto mt-3"> <?php echo is_null($Equipo) ?
                                            " Crear equipo " : "editar" ?></button>
                                </div>
                            </div>
                        </form>
